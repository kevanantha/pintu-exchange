...
import { Interval, KLineData, KlinesRespond, KlinesWsResponse } from "@/types";
...
export function CandlestickChart({ interval }: { interval: Interval }) {
  ...
  const fetchInitialData = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await binanceVisionInstance.get<Array<KLineData>>(
        `/uiKlines`,
        {
          params: {
            symbol: "BTCUSDT",
  ...
  useEffect(() => {
    if (lastMessage !== null && !isLoading) {
      const messageData: KlinesWsResponse = JSON.parse(lastMessage.data);

      if (messageData.k && messageData.k.i === interval) {
  ...
  return (
    <>
      <div className="tabular-nums max-md:flex-wrap flex gap-1.5 text-sm text-gray-600">
        <span>Open:</span>
        <span
